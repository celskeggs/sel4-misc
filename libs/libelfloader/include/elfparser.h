...
// both used in API for remap_cb and internally in the elf format
...
typedef seL4_Error (*remap_cb)(void *cookie, void *virt_target, uint8_t access_flags);
...
// once remap_cb is called, the memory should be pointing to the correct location (virt_target) and writeable.
// the access_flags value should be used AFTER the loading is complete.
extern seL4_Error elfloader_load(void *elf, size_t file_size, remap_cb remapper, void *cookie, void *page_buffer);
