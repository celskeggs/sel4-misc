...
static seL4_Error copy_via_remapper(void *source, size_t length, void *virt_target, elfparser_remap_cb remapper, void *cookie,
    ...
    while (length > 0) {
        ...
        seL4_Error err = remapper(cookie, page_base, access_flags);
        if (err != seL4_NoError) {
            return err;
        }
...
#define H32(n) (*(uint32_t *) (head + n))
    ...
    if (table_end <= table_pointer || entry_size < MIN_PHDR_SIZE ||
        table_end > end) { // detect overflow conditions; detect overread conditions
        return seL4_RangeError;
    }
    for (uint16_t i = 0; i < entry_count; i++) {
        ...
        if (stored_size > 0) {
            ...
            seL4_Error err = copy_via_remapper(read_source, stored_size, (void *) virtual_address, remapper, cookie,
                                               page_buffer, (uint8_t) (flags & ELF_MEM_FLAGS));
            if (err != seL4_NoError) {
                return err;
            }
        }
        ...
        for (uint32_t offset = 0; i < memory_size; i += PAGE_SIZE) {
            seL4_Error err = remapper(cookie, (void *) virtual_address + offset, (uint8_t) (flags & ELF_MEM_FLAGS));
            if (err != seL4_NoError) {
                return err;
            }
        }
